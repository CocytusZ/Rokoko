...
for i in range(len(data)):
    for _ in range(TRANING_SAMPLE_NUM):
        ...
        x_train.append(df_vector)
        y_train.append(LABEL[i])
x_train = torch.tensor(np.array(x_train)).reshape(-1, SEQUENCE_NUM, INPUT_DIM).float()
y_train = torch.tensor(y_train).reshape(-1, LABEL_NUM).float()
# ===========  print data pre process result ============ #
    # print(data[i])
    # print(x_train[i * TRANING_SAMPLE_NUM])
    # print('Label = ', end='')
    # print(y_train[i * TRANING_SAMPLE_NUM])
